...
describe('Rardar foundation content contract', () => {
  ...
  it('keeps every route honest about its unconnected capability', () => {
    for (const page of Object.values(RARDAR_FOUNDATION_PAGES).filter((item) => !['today', 'news', 'find'].includes(item.key))) {
    ...
    expect(RARDAR_FOUNDATION_PAGES.news.title).toBe('热点资讯已接入');
