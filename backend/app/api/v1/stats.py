...
async def get_source_distribution(days: int = Query(7, ge=1, le=90)):
    ...
    if cached:
        return cached
    return _cache_response(cache_key, query_stats_source_distribution(days=days))
...
async def get_category_distribution(days: int = Query(7, ge=1, le=90)):
    ...
    if cached:
        return cached
    return _cache_response(cache_key, query_stats_category_distribution(days=days))
...
async def get_daily_trend(days: int = Query(7, ge=1, le=90)):
    ...
    if cached:
        return cached
    return _cache_response(cache_key, query_stats_daily_trend(days=days))
...
async def get_novel_platform_stats():
    ...
    if cached:
        return cached
    return _cache_response(cache_key, query_stats_novel_platforms())
...
async def get_dashboard_stats(days: int = Query(7, ge=1, le=90)):
    ...
    if cached:
        return cached
    return _cache_response(cache_key, query_dashboard_stats(days=days))
