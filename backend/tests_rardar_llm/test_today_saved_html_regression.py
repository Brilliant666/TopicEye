...
def test_september_13_saved_union_preserves_eight_qualifiers_and_stale_source():
    snapshot = json.loads(
        (Path(__file__).parent / "fixtures" / "today-source-20260913.json").read_text(encoding="utf-8")
    )
    original = copy.deepcopy(snapshot)
    assert len(snapshot["projects"]) == 37
    for project in snapshot["projects"]:
        selected = select_growth(project["appearances"])
        assert ({"source": selected["source"], "value": selected["value"]} if selected else None) == project[
            "expectedPrimary"
        ]
    ...
    assert [project["primaryGrowth"]["value"] for project in result["projects"]] == [
        3680,
        3463,
        1015,
        729,
