...
- 隔离 production 预览从真实已保存双榜读取 GitHub 16 项、Trendshift 25 项、交集5项、并集36项；GitHub没有发布日字段，Trendshift日期为2026-09-11，采集时间保持原值。1440/390浏览器全量加载对照通过，非仅首页20项；104项历史列表包含明确标注的原Rardar历史榜依据。
- 同一 Archify 档案前后字段一致：4条能力、2项差异、9个入口与原生成时间/证据未变；未插入当前双榜。codecrafters 已生成解读保持原时间。真实新项目 `bilawalsidhu/gods-eye-view` 不在旧500项观察池，本轮经原collector用1次请求生成3条能力、2项差异和6个入口，Today/历史读取相同资料；再次执行直接复用，新增请求0。
- 从真实详情带入 Archify 到Find，登录后的实际需求提交完成10个真实GitHub候选与3项证据比较；桌面/手机无页面错误。此项仅为本轮接线验收，不新增推荐规则。
- 实际深度解读暴露原 `ReusableAsset.reuseType` 的 strict Python/JSON 不一致：合法枚举字符串会被误拒为 `is_instance_of`。实际控制入口回归先失败，再改为等价六值Literal后通过；没有放宽引用或内容规则，没有改模型与时限。原首次输出只存在运行进程内存，未声称恢复全部原响应；新增脱敏阶段/字段日志用于准确分类。
- CI浏览器的原完整并集、缺资料详情、GET零副作用、显式操作和重复提交用例全部保留，按当前前台名称与双榜scope更新夹具，并增加共享完整档案的实际组件树用例。最终CI、修正后真实解读及正式3000交付以PR收口记录为准。
