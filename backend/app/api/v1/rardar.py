...
from app.services.rardar_product import RardarProductError, explain_project, explain_project_by_id, find_projects
...
async def project_insight(
    ...
    try:
        ...
        if exc.code == "rardar_serving_project_not_found":
            status_code = 404
        elif exc.code in {"rardar_serving_source_invalid", "rardar_serving_source_not_found"}:
            status_code = 409
        else:
            status_code = 503
        raise HTTPException(status_code=status_code, detail={"code": exc.code}) from exc
