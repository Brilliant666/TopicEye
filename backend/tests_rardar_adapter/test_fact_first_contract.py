...
def _candidate(tmp_path: Path) -> ServingTodaySnapshot:
    ...
    for project in today.exactRanked:
        projects.append(project.model_copy(update={
            "materialState": "complete", "summarySource": "chinese_profile", "originalDescription": None,
...
def _without_material(project):
    return project.model_copy(update={
        "officialSummaryZh": None, "identitySummaryZh": None,
        "officialTaglineZh": None, "officialTaglineEvidenceRefs": [],
        "officialPositioningZh": None, "officialPositioningEvidenceRefs": [],
        "positioningZh": None, "positioningSourceMode": "insufficient", "positioningEvidenceRefs": [],
        "positioningIncludedRoles": [], "positioningExcludedClauses": [],
        "capabilities": [], "capabilityBulletsZh": [], "keyDifferentiators": [],
        "coreValueZh": None, "coreValueEvidenceRefs": [],
        "rardarAssessmentZh": None, "rardarAssessmentEvidenceRefs": [], "rardarDifferentiators": [],
        "officialHighlights": [], "materialState": "unavailable", "summarySource": "unavailable",
    })
...
def test_v8_missing_material_does_not_change_or_block_twenty_facts(tmp_path: Path, missing: int) -> None:
    ...
    candidate = today.model_copy(update={
...
def _collected(project, *, failures=(), refs=()):
    return SimpleNamespace(
        ...
        generation_failures=failures, deterministic_fallback_used=False,
        last_known_good_available=False, last_known_good_reused=False,
        current_evidence_fingerprint=None, last_known_good_fingerprint=None,
    )
...
def test_v8_unresolved_generation_failure_is_diagnostic_not_fact_gate(tmp_path: Path) -> None:
    ...
    profiles = SimpleNamespace(profiles={
        project.githubRepositoryId: _collected(project, failures=[failure]) for project in projects
    })
