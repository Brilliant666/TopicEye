...
def test_positioning_candidates_validate_before_source_preference() -> None:
    ...
    derived = candidate(
        ...
        ("readme:section:1",), "rardar_derived", ("core_mechanism", "primary_outcome"),
    )
    official_bad = candidate(
        "这是一个工具。搜索想做的事情，查看价格，然后调用它。",
        ("readme:section:1",), "official_translated", ("identity",),
    )
    official_good = candidate(
        "通过索引原始证据生成核查报告，帮助读者判断设备风险。",
        ("readme:section:1",), "official_translated",
    ...
    assert chosen == derived and reasons == ["positioning_candidate_official_translated_profile_serving_v6_positioning_requires_a_mechanism_or_primary_outcome"]
    ...
    assert chosen is None and reasons == ["positioning_candidate_official_translated_profile_serving_v6_positioning_requires_a_mechanism_or_primary_outcome"]
    ...
    duplicate = candidate("一套帮助读者分析设备风险的工具。", derived.evidence_refs, derived.source_mode, derived.included_roles)
