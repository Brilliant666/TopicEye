...
class ZhihuAlbum(Base):
    ...
    # 作者简介
    author_desc: Mapped[Optional[str]] = mapped_column(String(500), nullable=True)
    # 简介/摘要
    abstract: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    # 封面图 URL
    thumb_url: Mapped[Optional[str]] = mapped_column(String(1000), nullable=True)
    # 所属一级分类名
    ...
    # 文章/章节数（字符串如 "52 篇文章"）
    chapter_text: Mapped[Optional[str]] = mapped_column(String(100), nullable=True)
    # 价格（分，0 表示免费）
    ...
    # 上新时间（Unix timestamp）
    online_time: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)
    # 上新时间文本（如 "5 月 15 日上新"）
    online_time_text: Mapped[Optional[str]] = mapped_column(String(100), nullable=True)
    # 标签（独家/免费等）
    tag: Mapped[Optional[str]] = mapped_column(String(100), nullable=True)
    # 订阅类型（svip/free等）
    subscription_name: Mapped[Optional[str]] = mapped_column(String(50), nullable=True)
    ...
    # business_line（vip等）
    business_line: Mapped[Optional[str]] = mapped_column(String(50), nullable=True)
