...
def test_managed_config_rejects_unsafe_binding(tmp_path: Path, change: str) -> None:
    ...
    preview.mkdir()
    config = {
        "schemaVersion": 1, "repository": str(tmp_path), "backendPort": 54181,
        "frontendPort": 54180, "postgresPort": 55433,
        "dataDirectory": str(preview), "budgetIdentityDataDirectory": str(original),
        "database": "existing", "databaseUser": "existing_role",
    }
    if change == "port":
...
def test_managed_config_json_roundtrip_accepts_only_integer_ports(tmp_path: Path, port: object) -> None:
    ...
    path = tmp_path / "config.json"
    config = {
        "schemaVersion": 1, "repository": str(tmp_path), "backendPort": 54191,
        "frontendPort": port, "postgresPort": 55433,
        "dataDirectory": str(preview), "budgetIdentityDataDirectory": str(original),
        "database": "existing", "databaseUser": "existing_role",
    }
    path.write_text(json.dumps(config), encoding="utf-8")
...
def test_preview_build_directory_does_not_replace_runtime(setting: str, expected: str) -> None:
    ...
    result = subprocess.run(
        ...
        cwd=ROOT / "frontend", env=environment, capture_output=True, text=True,
